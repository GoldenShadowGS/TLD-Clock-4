	...
	m_Timer(timer)
{
	...
	m_Localtime.Init(factory, dc, 8.0f, -10.0f, D2D1::ColorF(0.48f, 0.43f, 0.54f, 1.0f), D2D1::ColorF(0.86f, 0.81f, 0.97f, 1.0f));
	HR(dc->CreateSolidColorBrush(D2D1::ColorF(0.75f, 0.79f, 0.75f, 0.25f), Brush.ReleaseAndGetAddressOf()));
	HR(dc->CreateSolidColorBrush(D2D1::ColorF(0.75f, 0.79f, 0.75f, 1.0f), HighlightBrush.ReleaseAndGetAddressOf()));
}
...
void UIBLocaltime::Draw(ID2D1DeviceContext* dc, ElementState state, BOOL focused)
{
	...
	INT64 currentTime = m_Timer->GetTime() / 5000;
	...
	dc->DrawEllipse(e, focused ? HighlightBrush.Get() : Brush.Get());
...
{
	return m_Timer->GetTime() / 5000 + m_Timeoffset;
}
