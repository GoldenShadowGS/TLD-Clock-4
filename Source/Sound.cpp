...
SoundManager::SoundManager(int numChannels, int reservedchannels)
{
	HRESULT hr = CreateMasterVoice();
	if (SUCCEEDED(hr))
	{
		m_NumChannels = numChannels;
		m_ReservedChannels = reservedchannels;
		SetWaveFormat();
		CreateChannels();
	}
	for (int i = SOUND_BEGIN + 1; i < SOUND_END; i++)
	{
		m_Sounds.insert({ i, WavFile(i) });
	}
}
...
void SoundManager::Play(int sound, float volume, float pitchratio)
{
	PlayWav(m_Sounds.at(sound), volume, pitchratio);
}

void SoundManager::Play(int sound, float volume, float pitchratio, int reservedchannel)
{
	PlayWav(m_Sounds.at(sound), volume, pitchratio, reservedchannel);
}

void SoundManager::PlayWav(WavFile& wavfile, float volume, float pitchratio)
{
	// Find an available Channel to play sound on
	int index = -1;
	for (int i = m_ReservedChannels ; i < m_NumChannels; i++)
	{
...
void SoundManager::PlayWav(WavFile& wavfile, float volume, float pitchratio, int reservedchannel)
{
	int index = -1;
	if (!m_Channels[reservedchannel].m_IsBusy)
	{
		index = reservedchannel;
		m_Channels[reservedchannel].m_IsBusy = true; // mark channel as in use
	}
...
void SoundManager::SetWaveFormat()
{
	...
	m_WaveFormat.wBitsPerSample = 16;  // 16
	m_WaveFormat.nBlockAlign = (m_WaveFormat.nChannels * m_WaveFormat.wBitsPerSample) / 8;
	m_WaveFormat.nSamplesPerSec = 44100;  // 44100  24000
	m_WaveFormat.nAvgBytesPerSec = 176400;  // 176400  96000
}
