...
{
public:
	SoundManager(int numChannels, int reservedchannels);
	~SoundManager();
	void Play(int sound, float volume, float pitchratio);
	void Play(int sound, float volume, float pitchratio, int reservedchannel);
private:
	void PlayWav(WavFile& wavfile, float volume, float pitchratio);
	void PlayWav(WavFile& wavfile, float volume, float pitchratio, int reservedchannel);
	void SetWaveFormat();
	...
	int m_NumChannels = 0;
	int m_ReservedChannels = 0;
	std::map<int, WavFile> m_Sounds;
};